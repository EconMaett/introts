...
plot(freqg, 10 * log10(xg.spec$spec[4400:5600]),
  main = "",
...
plot(Frequ, Sord,
  main = "Spectrum", xlab = "Frequency (Hz)",
...
# plot in Figure 10.1, etc.  
# Note that this one takes a while to simulate!
m <- 1
c <- 1
k <- 16.25
Delta <- 0.01
a0 <- m / Delta^2 + c / Delta + k
a1 <- -2 * m / Delta^2 - c / Delta
a2 <- m / Delta^2
n <- 100000
y <- c(0, 0)
x <- c(0, 0)
...
attach(tug.dat)
Heave.spec <- spectrum(Heave,
  span = sqrt(length(Heave)),
  log = c("no"), main = ""
)
...
Salt.dat <- read.table(www, header = T)
attach(Salt.dat)
n <- 81
Time <- 1:n
...
m1 <- SS(
  ...
  Fmat =
    function(tt, x, phi) {
      return(matrix(
        c(
          x[tt, 1], x[tt, 2], x[tt, 3], x[tt, 4], x[tt, 5], rep(0, 10),
